...
\begin{lem}
Kompaktien joukkojen mielivaltainen tulo on kompakti.
\begin{proof}
Topologia II \cite{Topo2} Lause ?.?.
\end{proof}
\end{lem}
...
Toiseksi kuvauksen $\rho$ komponenttikuvaukset ovat joukon $I^X$ alkioina jatkuvia. 
Näin ollen myös kuvaus $\rho$ on jatkuva \cite[lause~?.?]{Topo2}. 
%$(P_f\circ\rho)(x)
%%=P_f(\rho(x))
%=f(x)$ kaikilla $x\in X$ ja $f\in I^X$ yhdisteenä jatkuva.
...
$(U_f)_{f\in I^X}$, ${U_f=f^\leftarrow [0,1[\subset X}$ kaikilla $ f\in I^X$ 
%\begin{equation*}
...
Tällöin perhe $(U_f)_{f\in I^X}$ on ympäristökanta \cite[lause~?.?]{Topo2}. 
...
on avoin joukossa $\rho X$ ja näin ollen kuvaus $\rho$ on avoin \cite[lause~?.?]{Topo2}.
\end{proof}
\end{lause}
...
Avaruus $\beta X$ on kompakti, joten $F(\beta X)$ on suljettu joukko, 
