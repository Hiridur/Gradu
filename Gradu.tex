...
Olkoon $X$ joukko ja $f$ pseudomeriikka joukolle $X$. Tällöin pseudometriikka $f$ määrittää sellaisen uniformiteetin joukolle $X$, jonka kannan muodostaa kokoelma $B=\{ f^{\rightarrow}[0,a]\mid a\in\R_+\}$.
\begin{proof}
Olkoon $a\in\R_+$ reaaliluku ja merkitään 
joukkoa $f^{\rightarrow}[0,a]\subset X\times X$ kaavalla $f^{\rightarrow}[0,a]=\U_a$. 
Määritelmän \ref{uniformin kannan maar} ehdot pätevät:
\begin{enumerate} [label=(B\arabic*)]
\item %\ref{B_I} 
...
%kokoelmat $B_f=\{ f^{\rightarrow}[0,a]\mid a\in\R_+\}$ ja $B_g=\{ g^{\rightarrow}[0,a]\mid a\in\R_+\}$ ovat uniformiteettien kantoja joukolle $X$. 
