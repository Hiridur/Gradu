...
jos identtinen kuvaus $id\colon (X,\U_1)\rightarrow (X,\U_2)$ on uniformisti jatkuva. Tällöin $\U_1$ on hienompi kuin $\U_2$. 
...
kannan jäsen $g^{\leftarrow}[0,b'],b'\in\R_+$ niin, 
että $(x,y)\in g^{\leftarrow}[0,b']\Rightarrow (x,y)\in f^{\leftarrow}[0,a']$, kaikilla $x,y\in X$. 
Näin ollen identtinen kuvaus $id\colon (X,\U_g)\rightarrow (X,\U_f)$ on 
uniformisti jatkuva ja siten uniformiteetti $\U_g$ on uniformiteettia $\U_f$ hienompi. 
...
\begin{proof}
Jokaiselle $U\in\U$ määritellään perhe $(V_n)$, 
...
\end{equation}
koska $\sum_{i<h+1}g(z_i,z_{i+1})> a/2$. 
...
\end{equation*}
ja toisaalta %sijoituksella "$0=h+1$"
\begin{equation*}
\frac{1}{2}a\geq \sum_{i<h}g(z_i,z_{i+1})\geq \frac{1}{2}\,g(z_{h+1},y).
\end{equation*}
Tällöin kaavojen %\ref{numeroituvakantapm2}, 
\ref{numeroituvakantapm3} ja \ref{numeroituvakantapm4} nojalla $g(x,z_h)\leq a$, $g(z_{h+1},y)\leq a$ ja 
