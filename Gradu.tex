...
\sum_{i<h}g(z_i,z_{i+1})\leq a/2\quad\text{ ja }\quad
\sum_{i<h+1}g(z_i,z_{i+1})> a/2,
\end{equation}
joten $\sum_{i>h}g(z_i,z_{i+1})\leq a/2$. 
%TODO tästä eteenpäin vaatii viimeistelyä
Kaavojen \ref{numeroituvakantapm2}, \ref{numeroituvakantapm3} ja \ref{numeroituvakantapm4} nojalla $g(x,z_h)\leq a$, $g(z_{h+1},y)\leq a$ ja 
