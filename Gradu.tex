...
$\{g_{A_1},g_{A_2},\dots,g_{A_n}\}\subset (g_{H})$ supremum 
%$\sup_{j=1}^{n}g_{A_j}$ 
...
Lauseen \ref{saturoitu lause} tilanteessa sanotaan, 
että pseudometriikkaperhe $(g_H)$ on \emph{saturoitu} (saturated) ja 
%että perhe $(g_H)$ on 
saatu saturoimalla perhe $(f_i)_{i\in I}$. 
\end{maar}
\begin{lause}
...
\end{maar}
\begin{lause}
Olkoon $X$ joukko ja $\F$ ja $\F'$ filttereitä joukolle $X$. 
Olkoon $B$ filtterin $\F$ kanta ja $B'$ filtterin $\F'$ kanta. 
Filtteri $\F'$ on hienompi kuin filtteri $\F'$ jos ja vain jos 
jokaiselle $M\in B$ löytyy 
...
kuvaus. 
Jos kuvaus $f$ on jatkuva, %(topologian $\T$ suhteen), 
